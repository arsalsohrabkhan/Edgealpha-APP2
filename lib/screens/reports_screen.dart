                  ...
                  child: SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      rows: C.trades.map((t) => DataRow(cells: [
                        DataCell(Text(t.asset,
                            style: AETheme.syne(size: 12, weight: FontWeight.w800))),
                        DataCell(Text(t.isLong ? '▲ Long' : '▼ Short',
                            style: AETheme.syne(size: 10,
                                color: t.isLong ? AETheme.green : AETheme.red,
                                weight: FontWeight.w700))),
                        DataCell(Text(
                            '${t.pnl >= 0 ? '+' : '−'}${fmt.format(t.pnl.abs())}',
                            style: AETheme.syne(size: 12,
                                color: t.isWin ? AETheme.green : AETheme.red,
                                weight: FontWeight.w700))),
                        DataCell(Text(
                            '${t.pct >= 0 ? '+' : ''}${t.pct}%',
                            style: AETheme.syne(size: 12,
                                color: t.pct >= 0 ? AETheme.green : AETheme.red))),
                        DataCell(Text(t.openDate,
                            style: AETheme.syne(size: 10, color: AETheme.muted))),
                        DataCell(Text(t.closeDate,
                            style: AETheme.syne(size: 10, color: AETheme.muted))),
                      ])).toList(),
                    ),
                  ),
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            flex: 3,
                            child: Text(r[1],
                                style: AETheme.syne(size: 13, weight: FontWeight.w700)),
                          ),
                        ],
                      ),
                    )).toList(),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(widget.title,
                      style: AETheme.syne(size: 15, weight: FontWeight.w800)),
                  const SizedBox(height: 8),
                  Text(widget.desc,
                      style: AETheme.syne(
                          size: 12, color: AETheme.slate, weight: FontWeight.w400)),
    ...
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
              child: Text(title,
                  style: AETheme.fraunces(size: 15, color: Colors.white),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Text(
              'AlphaEdge Capital · Private & Confidential · ${DateTime.now().year}',
