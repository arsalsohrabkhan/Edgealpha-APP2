      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            Row(
              children: [
                _MetricCard(label: 'Sharpe Ratio',     value: C.sharpe.toStringAsFixed(2), note: 'Annualised · simplified model', color: AETheme.indigo2, gradient: AETheme.indigoGradient),
            ...
            _CardWrap(
              ...
              child: Padding(
                ...
                child: EquityLineChart(equity: C.equity, months: C.months, height: 220),
              ),
            ),
    ...
    return Expanded(
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(label.toUpperCase(), style: AETheme.syne(size: 9, color: AETheme.muted, weight: FontWeight.w800, letterSpacing: 1.2)),
            const SizedBox(height: 8),
            ShaderMask(
              shaderCallback: (b) => gradient.createShader(b),
              child: Text(value, style: AETheme.fraunces(size: 36, color: Colors.white, letterSpacing: -2)),
            ),
            const SizedBox(height: 5),
            Text(note, style: AETheme.syne(size: 10, color: AETheme.muted, weight: FontWeight.w500)),
          ],
        ),
      ),
    );
...
class _LongShortBar extends StatelessWidget {
  ...
  const _LongShortBar({
    required this.longCount, required this.shortCount,
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        Row(children: [
          Expanded(
            child: _SideStat(label: 'Long', count: longCount, pnl: longPnl, fmt: fmt, color: AETheme.green, isLeft: true),
          ),
          Expanded(
            child: _SideStat(label: 'Short', count: shortCount, pnl: shortPnl, fmt: fmt, color: AETheme.red, isLeft: false),
          ),
        ]),
...
class _SideStat extends StatelessWidget {
  ...
  const _SideStat({required this.label, required this.count, required this.pnl, required this.fmt, required this.color, required this.isLeft});
